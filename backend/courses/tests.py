...
class EnrollmentModelTest(TestUserCourseEnrollmentMixin, CourseImageCleanupTestMixin, TestCase):
    ...
    def test_enrollment_respects_max_attendants(self):
        course = self.create_test_course(max_attendants=1)
        first_user = self.create_test_user(email='first@example.com', username='firstuser')
        second_user = self.create_test_user(email='second@example.com', username='seconduser')
        Enrollment.objects.create(user=first_user, course=course)
        with self.assertRaises(ValidationError):
            Enrollment.objects.create(user=second_user, course=course)
...
class CourseViewSetTest(CourseImageCleanupTestMixin, APITestCase):
    ...
    def test_duplicate_course_as_admin(self):
        ...
        duplicated_course = Course.objects.get(id=response.data['id'])
        self.assertEqual(duplicated_course.enrollments.count(), 0)
    ...
    def test_stripe_webhook_branches(self):
        ...
        event = {
            ...
            "data": {"object": {"metadata": {"user_id": str(self.user.id),
                                             "course_id": str(self.course.id)}, "payment_intent": "pi_1"}},
        }
        with patch("courses.views.stripe.Webhook.construct_event", return_value=event):
            resp = self.client.post(url, data=b"{}", content_type="application/json")
        self.assertEqual(resp.status_code, 200)
        ...
        # Ensure we hit the creation branch by removing the existing enrollment
        ...
        with patch("courses.views.stripe.Webhook.construct_event", return_value=event), patch(
            "courses.views.Enrollment.objects.create", side_effect=Exception("boom")
        ):
            resp = self.client.post(url, data=b"{}", content_type="application/json")
