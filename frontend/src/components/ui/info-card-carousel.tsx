...
function InfoCardMedia({ item }: { item: InfoCardItem }) {
  const showHoverEffect = hasCardCopy(item);
...
function InfoCardTextContent({ item }: { item: InfoCardItem }) {
  return (
    <div className="relative z-10 flex h-full flex-col items-start justify-start gap-4 p-8 text-left">
      ...
      {item.description && (
        <p className="text-lg italic leading-relaxed text-slate-medium dark:text-cloud-medium">
          {item.description}
        </p>
      )}
      {item.ctaLabel && (
        <span className="mt-2 inline-flex items-center text-sm font-semibold not-italic text-clay">
          {item.ctaLabel}
        </span>
      )}
    </div>
    ...
    <>
      ...
      {showOverlay && (
        <div className="relative z-10 flex h-full flex-col justify-end p-5">
          ...
          {item.ctaLabel && (
            <span className="mt-3 inline-flex items-center text-sm font-semibold text-white">{item.ctaLabel}</span>
          )}
        </div>
      )}
    </>
